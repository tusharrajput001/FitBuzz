...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        const { workout_days, meals, name, id } = req.body;
        const userId = req.user.id;

        console.log('Extracted workout_days:', workout_days);
        console.log('Extracted meals:', meals);
        console.log('Extracted name:', name);
        console.log('Extracted id:', id);
        console.log('User ID:', userId);

        if (!workout_days || !meals) {
            console.log('Validation failed - missing workout_days or meals');
            return res.status(400).json({ 
                error: 'Workout days and meals are required',
                received: {
                    hasWorkoutDays: !!workout_days,
                    hasMeals: !!meals,
        ...
        console.log(`Processing ${workout_days.length} workout days...`);
        for (let i = 0; i < workout_days.length; i++) {
            const workoutDay = workout_days[i]; 
            ...
            const { data: day, error: dayError } = await supabase
                ...
                .insert({
                    plan_id: planId,
                    day_number: workoutDay.day_number || i + 1,
                    day_name: workoutDay.day_name || `Day ${i + 1}`,
                    focus_area: workoutDay.focus_area || 'General Fitness'
                })
                .select()
                .single();
            ...
            console.log(`Workout day ${i + 1} exercises:`, workoutDay.workout_exercises);
            console.log(`Exercises array length:`, workoutDay.workout_exercises?.length);
            
            if (workoutDay.workout_exercises && workoutDay.workout_exercises.length > 0) {
                const exercises = workoutDay.workout_exercises.map(exercise => {
        ...
        // meals is directly an array of meals
        for (let i = 0; i < meals.length; i++) {
            const meal = meals[i];
            console.log(`Processing meal ${i + 1}:`, JSON.stringify(meal, null, 2));
            
            // Create meal
            console.log(`Creating meal: ${meal.meal_type} for plan ${planId}`);
            ...
            // Insert meal items
            console.log(`Meal items for ${meal.meal_type}:`, meal.meal_items);
            
            if (meal.meal_items && meal.meal_items.length > 0) {
                const mealItems = meal.meal_items.map(item => {
                ...
                console.log(`No meal items found for meal ${meal.meal_type}`);
            }
        }
